...
router.post('/webhooks', (req, res) => {
  ...
  const wh = webhooks.addWebhook({ name, url, type: type || 'custom', minSeverity: minSeverity || 'high' })
  res.json(wh)
})
...
router.post('/webhooks/test/:id', async (req, res) => {
  ...
  if (!wh) return res.status(404).json({ error: 'not found' })
  const testEvent = { blocked: true, severity: 'high', ruleName: 'Test Alert', category: 'test', ip: '1.2.3.4', country: 'XX', url: '/test', reason: 'WAF test alert', timestamp: new Date().toISOString() }
  try {
