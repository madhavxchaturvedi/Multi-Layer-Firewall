...
function DashboardApp() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => { dispatch(fetchStats()); dispatch(fetchSuspicious()) }, 10000)
...
export default function App() {
  // Show landing first; "entered" flips after CTA click
  const [showDash, setShowDash] = useState(false)

  if (!showDash) {
    return <Landing onEnter={() => setShowDash(true)} />
  }
