...
import { allowIP, simulateAttack } from "../store/wafSlice";
  ...
  return (
    <div className="flex h-screen overflow-hidden animate-fade-in">
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-1 bg-surface-2 rounded-lg p-1">
            {["all", "blocked", "allowed"].map((f) => (
              <button
                ...
                onClick={() => setFilter(f)}
                className={cn(
                  "px-3 py-1 rounded text-xs font-mono uppercase tracking-wide transition-colors",
                  filter === f
        ...
        {fpMsg && (
          <div className="mx-6 mt-3 px-4 py-2 bg-accent-green/10 text-accent-green border border-accent-green/20 rounded-lg text-xs font-mono">
            {fpMsg}
          </div>
        )}
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((ev, i) => (
              <div
                ...
                className={cn(
                  "px-6 py-2.5 flex items-center gap-4 cursor-pointer transition-colors text-xs hover:bg-surface-2",
                  selected?.id === ev.id &&
                    "bg-surface-2 border-l-2 border-accent-red",
                  ev.blocked && i < 3 && "animate-slide-in",
                )}
              >
                <SeverityDot severity={ev.severity} />
                <span className="font-mono text-gray-600 w-20 flex-shrink-0">
                ...
                <span className="font-mono text-gray-600 w-28 truncate hidden lg:block flex-shrink-0">
                  {ev.ip}
                </span>
                <span className="font-mono text-gray-600 w-10 flex-shrink-0">
                  {ev.country || "—"}
                </span>
              </div>
            ))
        ...
        <div className="w-80 border-l border-border bg-surface-1 overflow-y-auto flex-shrink-0 animate-slide-in">
          ...
          <div className="p-5 space-y-4">
            ...
            {[
              ...
              [
                "User Agent",
                selected.userAgent?.slice(0, 60) +
                  (selected.userAgent?.length > 60 ? "…" : ""),
              ],
            ].map(([k, v]) => (
              <div key={k}>
            ...
            {selected.blocked && (
              <div className="border-t border-border pt-4 space-y-2">
                <div className="text-[10px] font-mono text-accent-red uppercase tracking-widest mb-2">
                  Attack Info
                </div>
                {[
            ...
            <div className="border-t border-border pt-4 space-y-2">
              ...
              {replayResult && (
                ...
                >
                  Replay: HTTP {replayResult.status} —{" "}
                  {replayResult.blocked
                    ? "Blocked ✓"
                    : "Passed (rule may be disabled)"}
                </div>
              )}
