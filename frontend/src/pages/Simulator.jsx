...
const ATTACKS = [
  {
    id: 'sqli', label: 'SQL Injection', color: 'red',
  ...
  {
    id: 'xss', label: 'XSS Attack', color: 'orange',
    icon: '◈', severity: 'high',
  ...
  {
    id: 'path', label: 'Path Traversal', color: 'yellow',
    icon: '⊛', severity: 'high',
  ...
  {
    id: 'cmdi', label: 'Command Injection', color: 'purple',
    icon: '⊕', severity: 'critical',
  ...
  {
    id: 'scanner', label: 'Scanner Bot', color: 'gray',
  ...
  {
    id: 'honeypot', label: 'Honeypot Trip', color: 'red',
    icon: '⚠', severity: 'critical',
  ...
  {
    id: 'blind_sqli', label: 'Blind SQLi', color: 'red',
    icon: '⬡', severity: 'critical',
...
const COLOR_MAP = {
  ...
  orange: 'border-accent-orange/30 bg-accent-orange/5 hover:bg-accent-orange/10 text-accent-orange',
  yellow: 'border-accent-yellow/30 bg-accent-yellow/5 hover:bg-accent-yellow/10 text-accent-yellow',
...
export default function Simulator() {
  ...
  const { simulationResult, events } = useSelector(s => s.waf)
  ...
  return (
    <div className="p-6 animate-fade-in space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="font-display font-semibold text-white text-lg">Attack Simulator</div>
          <div className="text-xs font-mono text-gray-500 mt-0.5">Fire test attacks against the WAF — watch them get caught in the live feed</div>
        </div>
        <button
      ...
      {results.length > 0 && (
        <div className="flex items-center gap-6 bg-surface-1 border border-border rounded-xl px-5 py-3">
          <div className="text-center">
            <div className="text-2xl font-display font-bold text-accent-red">{blockedCount}</div>
            <div className="text-[10px] font-mono text-gray-600">Blocked</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-display font-bold text-accent-green">{results.length - blockedCount}</div>
          ...
          <div className="text-center">
            ...
            <div className="text-[10px] font-mono text-gray-600">Detection Rate</div>
          </div>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {firing === attack.id && (
                  <span className="text-[10px] font-mono text-gray-400 animate-pulse">firing…</span>
                )}
              </div>
              <div className="font-display font-semibold text-sm mb-1">{attack.label}</div>
              <div className="text-[11px] font-sans opacity-70">{attack.description}</div>
              <div className="mt-2 text-[10px] font-mono opacity-50 truncate">{attack.payload}</div>
            </button>
          )
        })}
      </div>
      ...
      <div className="bg-surface-1 border border-border rounded-xl p-5">
        <div className="text-xs font-mono text-gray-500 uppercase tracking-widest mb-3">Custom Payload</div>
        <div className="flex gap-3 flex-wrap">
          ...
          />
          <input
            value={customPayload} onChange={e => setCustomPayload(e.target.value)}
          ...
          />
          <button onClick={fireCustom} disabled={!!firing || !customPayload}
      ...
      {results.length > 0 && (
        <div className="bg-surface-1 border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-border text-[10px] font-mono text-gray-600 uppercase tracking-widest">Simulation Log</div>
          <div className="divide-y divide-border">
            {results.map((r, i) => (
              <div key={r.id} className="px-5 py-2.5 flex items-center gap-4 text-xs animate-slide-in">
                <span className={cn('text-[10px] font-mono px-1.5 py-0.5 rounded border',
                  r.blocked ? 'text-accent-green border-accent-green/30 bg-accent-green/10' : 'text-accent-red border-accent-red/30 bg-accent-red/10'
                )}>
                  {r.blocked ? 'BLOCKED' : 'PASSED'}
                </span>
                <span className="font-mono text-gray-400 w-28 flex-shrink-0">{r.attack}</span>
                <span className="font-mono text-gray-600 flex-1 truncate">{r.payload}</span>
                <span className="font-mono text-gray-700 flex-shrink-0">HTTP {r.status}</span>
              </div>
            ))}
          </div>
        </div>
