...
export default function WorldMap() {
  ...
  const tooltip = hovered
    ? (() => {
        const PAD = 8,
          ...
          TH = 76;
        const tx =
          hovered.x + 14 + TW > size.w ? hovered.x - TW - PAD : hovered.x + PAD;
        const ty =
          hovered.y - TH / 2 < 0
            ? PAD
            : hovered.y + TH > size.h
  ...
  return (
    <div className="p-6 animate-fade-in space-y-5">
      ...
      <div
        ref={containerRef}
        className="bg-surface-1 border border-border rounded-xl overflow-hidden relative select-none"
      >
        {/* Canvas: land shapes */}
        <canvas
          ref={canvasRef}
          ...
          {hovered && tooltip && (
            <g>
              ...
              <rect
                ...
                height={76}
                rx={1.5}
                fill={SEV_COLOR[hovered.severity] || "#ff3b3b"}
              />
              <text
                x={tooltip.tx + 12}
                y={tooltip.ty + 16}
              ...
              <text
                ...
                fontFamily="monospace"
              >
                {hovered.url?.slice(0, 23)}
                {hovered.url?.length > 23 ? "…" : ""}
              </text>
              <text
                x={tooltip.tx + 12}
                y={tooltip.ty + 61}
        ...
        {!geo && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0b1829]">
            <div className="text-gray-500 text-xs font-mono animate-pulse">
              Loading world map…
            </div>
          </div>
        )}
        ...
        {geo && dots.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <div className="text-gray-600 text-xs font-mono">
              No blocked attacks plotted yet
            </div>
            <div className="text-gray-700 text-[10px] font-mono mt-1">
              Go to Simulator → Fire All Attacks to see dots appear
            </div>
          </div>
        )}
      </div>
            ...
            {dots.length === 0 ? (
              ...
              dots.slice(0, 7).map((d, i) => (
                ...
                >
                  ...
                  />
                  <span className="font-mono text-gray-400 w-8 flex-shrink-0">
                    {d.country}
                  </span>
                  <span className="font-mono text-gray-500 flex-1 truncate">
                    {CATEGORY_LABEL[d.category] || d.category || "—"}
                  </span>
                  <span className="font-mono text-gray-600 flex-shrink-0 truncate max-w-[90px]">
                    {d.ip}
                  </span>
                </div>
              ))
